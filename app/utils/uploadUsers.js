...
module.exports = {
  addPicture(file) {
    ...
    const photoUrl = path.resolve(__dirname, "../../public/users/", fileName);
  ...
  updatePicture(file, user) {
    ...
    const pathPhoto = `./public/users/${user.picture}`;
    fs.unlinkSync(pathPhoto);
    newPicture.mv(`./public/users/${photo}`, (err) => {
      if (err) {
        throw {
          name: err.name,
          message: err.message,
